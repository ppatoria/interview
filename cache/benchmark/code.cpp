...
void ProcessDataWithBuffering(const std::vector<DataType>& updates, benchmark::State& state) {
    std::array<DataType, BufferSize> buffer;

    for (auto _ : state) {
        for (std::size_t i = 0; i < updates.size(); i += BufferSize) {
            std::size_t chunk_size = std::min(BufferSize, updates.size() - i);
